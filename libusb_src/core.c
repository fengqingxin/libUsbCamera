	...
	libusb_device ***list)
{
	...
	if (libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)) {
	    ...
	    usbi_dbg("Not Supported Hotplug\n");
	...
	libusb_device_handle **dev_handle)
{
    ...
    usbi_dbg("[usbi_backend.open]");
	r = usbi_backend.open(_dev_handle);
	if (r < 0) {
		usbi_dbg("[usbi_backend.open]open %d.%d returns %d", dev->bus_number, dev->device_address, r);
		libusb_unref_device(dev);
		usbi_mutex_destroy(&_dev_handle->lock);
		free(_dev_handle);
		usbi_dbg("[usbi_backend.open]open: %d", r);
		return r;
	}
...
int API_EXPORTED libusb_init(libusb_context **context)
{
	...
	if (usbi_backend.init) {
		...
		if (r)  {
            usbi_dbg("usbi_backend.init failed\n");
			goto err_free_ctx;
        }
	}usbi_dbg("usbi_backend.init succ\n");

	r = usbi_io_init(ctx);
	if (r < 0)  {
        usbi_dbg("usbi_io_init failed\n");
		goto err_backend_exit;
    }usbi_dbg("usbi_io_init succ\n");
