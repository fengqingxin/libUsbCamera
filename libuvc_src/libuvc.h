...
typedef struct uvc_frame {
  ...
  uint32_t height;
  /** Flag indicate whether this image is still or normal in buffer,  add yuliang 20190830 */
  uint8_t ucImageType;  // 0 for normal; 1 for still
  /** Pixel data format */
...
typedef struct uvc_stream_ctrl {
  ...
  struct  uvc_still_ctrl* pStillCtrl;  // add by yuliang
} uvc_stream_ctrl_t;

/* add by yuliang 20190416 */
typedef struct uvc_StatusCallBack
{
  void* pCameraControl;
  uint8_t bStatus;
  uint8_t bStatusType;
...
uint8_t uvc_get_port_number(uvc_device_t* dev);
int uvc_IsUsbOpened(uvc_context_t *ctx, uvc_device_t* dev);
...
void uvc_break(uvc_device_handle_t *devh);
