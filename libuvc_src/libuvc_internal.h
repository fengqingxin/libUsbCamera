...
#define UVC_DEBUG(format, ...) printf("[%s:%d/%s] " format "\n", __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#define UVC_ENTER() printf("[%s:%d] begin %s\n", __FILE__, __LINE__, __FUNCTION__)
#define UVC_EXIT(code) printf("[%s:%d] end %s (%d)\n", __FILE__, __LINE__, __FUNCTION__, code)
#define UVC_EXIT_VOID() printf("[%s:%d] end %s\n", __FILE__, __LINE__, __FUNCTION__)
...
/* override it by yuliang 20200430  */
...
#define UVC_XU_CONTROL_USER_CMD_PROP_ID                     (0x01U)    // 用户命令接口
#define UVC_XU_CONTROL_UPDATE_PKG_PROP_ID                   (0x02U)    // 更新包接口
#define UVC_XU_CONTROL_DECODE_RESULT_LONG_ID                (0x03U)    // 设备信息接口
...
struct uvc_stream_handle {
  ...
  /** Current control block */
  struct uvc_still_ctrl cur_Stillctrl;

  /* listeners may only access hold*, and only when holding a
   * lock on cb_mutex (probably signaled with cb_cond) */
  uint8_t fid;
  uint8_t flgStillFinish;
  uint8_t isStill;  // add by yuliang 20190829
  uint32_t seq, hold_seq;
  uint32_t pts, hold_pts;
  uint32_t last_scr, hold_last_scr;
  size_t got_bytes, hold_bytes;
  size_t got_still_bytes;   // add by yuliang, still capture numbers
  uint8_t *outbuf, *holdbuf, *stillOutBuf;  // add by yuliang
