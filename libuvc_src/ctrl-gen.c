...
#if 0
/** @ingroup ctrl
 * @brief Reads the UVC_XU_CONTROL_SONIX_ASIC_RW control.
 * @param devh UVC device handle
 * @param[out] selector TODO
 * @param req_code UVC_GET_* request to execute
 */
uvc_error_t uvc_get_xu_asic(uvc_device_handle_t *devh, uint16_t addr, uint8_t* asicData) {
  uint8_t xu_data[4];
  uvc_error_t ret;

  memset(xu_data,0,sizeof(xu_data));
  xu_data[0] = (addr & 0xFF);
...
/** @ingroup ctrl
 * @brief Sets the UVC_XU_CONTROL_SONIX_ASIC_RW control.
 * @param devh UVC device handle
 * @param selector TODO
 */
uvc_error_t uvc_set_xu_asic(uvc_device_handle_t *devh, uint16_t addr, uint8_t asicData) {
  uint8_t xu_data[4];
  uvc_error_t ret;
...
/** @ingroup ctrl
 * @brief Reads the UVC_XU_CONTROL_SONIX_FLASH_CTRL control.
 * @param devh UVC device handle
 * @param[out] selector TODO
 * @param req_code UVC_GET_* request to execute
 */
uvc_error_t uvc_get_xu_flash(uvc_device_handle_t *devh, uint32_t addr, uint8_t length, uint8_t* asicData) {
  uint8_t xu_data[11];
  uvc_error_t ret;
  ...
  xu_data[0] = addr & 0xff;
  xu_data[1] = (addr >> 8) & 0xff;
  xu_data[2] = (0x80 + ((addr >> 12) & 0xf0)) | length;

  ret = libusb_control_transfer(
    devh->usb_devh,
    REQ_TYPE_SET, UVC_SET_CUR,
    UVC_XU_CONTROL_SONIX_FLASH_CTRL << 8,
    uvc_get_extension_units(devh)->bUnitID << 8 | devh->info->ctrl_if.bInterfaceNumber,
    xu_data,
    sizeof(xu_data),
    0);
  ...
  if (ret == sizeof(xu_data)) {
    memcpy(asicData, xu_data + 3, length);
    return UVC_SUCCESS;
  } else {
    return ret;
  }
...
/** @ingroup ctrl
 * @brief Reads the UVC_XU_CONTROL_DATA control.
 * @Func  uvc_get_xu_data()
 * @param devh UVC device handle
 *        pData,
          DataLen,
 * @param[out] selector TODO
 ...
 * @Add  by yuliang
 */
uvc_error_t uvc_get_xu_data(uvc_device_handle_t *devh, uint8_t cs, uint8_t* pData, uint16_t DataLen) {
  uint8_t* xu_data = NULL;
  uvc_error_t ret;

  if(DataLen <=0)
	  return UVC_ERROR_INVALID_PARAM;

  xu_data = (uint8_t*)malloc(DataLen);
  if(xu_data == NULL) {
	  return UVC_ERROR_NO_MEM;
  } else {
      memset(xu_data, 0, DataLen);
  }

  ret = libusb_control_transfer(
    devh->usb_devh,
    REQ_TYPE_GET, UVC_GET_CUR,
    cs << 8,  /* The wValue field specifies the Control Selector (CS) in the high byte and zero in the low byte.  */
    uvc_get_extension_units(devh)->bUnitID << 8 | devh->info->ctrl_if.bInterfaceNumber,
    xu_data,
    DataLen,
    0);
...
/** @ingroup ctrl
 * @brief Sets the UVC_XU_CONTROL_Data_RW control.
 * @param devh UVC device handle
 * @param selector TODO
 * @Add  by yuliang
 */
uvc_error_t uvc_set_xu_data(uvc_device_handle_t *devh, uint8_t cs, uint8_t* pData, uint16_t DataLen) {
  uvc_error_t ret;
 ...
 * @param state TODO
 */
uvc_error_t uvc_get_ct_max(uvc_device_handle_t *devh, uint8_t* pbData, uint8_t bLen)
{
  uint8_t data[10];
  uvc_error_t ret;
  ...
  ret = libusb_control_transfer(
    devh->usb_devh,
    REQ_TYPE_GET, UVC_GET_MIN,
    UVC_CT_AE_MODE_CONTROL << 8,  /* The wValue field specifies the Control Selector (CS) in the high byte and zero in the low byte.  */
    uvc_get_camera_terminal(devh)->bTerminalID << 8 | devh->info->ctrl_if.bInterfaceNumber,
    data,
    bLen,
    0);
